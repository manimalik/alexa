...
class Dashboard extends Component {	
	...
	handleSubmit = (event) => {
	    fetch('https://briefify.coldsmoke.co/alex/files/submit_progress.php', {
		  method: 'POST',
		  headers: {
	...
	handleChange = (newValue) => {
		...
		fetch('https://briefify.coldsmoke.co/alex/files/getProgress.php', {
		  method: 'POST',
		  headers: {
		    Accept: 'application/json',
		    'Content-Type': 'application/json',
	...
	render() {
	    ...
	    if (cachedHits === null) {
	    	window.location.href = "/alex/admin";
	    }
	    if( this.state.count === 0 ) {
		    fetch('https://briefify.coldsmoke.co/alex/files/fetch_shops.php', {
			  method: 'GET',
			  headers: {
			    'Cache-Control': 'no-cache, no-store, must-revalidate',
		    	'Pragma': 'no-cache',
